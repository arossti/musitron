...
class ArpPlayer {
  constructor(params) {
    ...
    this.progressions = {
      simple: [0,2,4,2],
      glass_1: [0,2,6,3,4,2,5,1],
      glass_2: [0,4,2,6,1,5,3,0],
    ...
    this.composition_structure = [
      ...
      { name: 'building', duration: 32, layers: 2, tempo_mod: 1.0 },
      { name: 'complexity', duration: 48, layers: 3, tempo_mod: 1.2 },
  ...
  _loadTransport() {
    ...
    Tone.Transport.scheduleRepeat((time) => {
      ...
      let pattern_multiplier = section.layers;
      let notes = chord.triad.notes;
      for(let i = 0; i < Math.ceil(this.ap_steps / 3) * pattern_multiplier; i++) {
      ...
      let pattern_variation = this.getPatternVariation(section, this.section_step);
      let note_index = (this.arpeggio[this.player.step % this.arpeggio.length] + pattern_variation) % notes.length;
      ...
      // Change sections
      if(this.section_step >= section.duration * 4) { // 4 steps per beat
 ...
 */
  ...
  _loadProgressionSelector() {
    let progression_container = document.createElement('section');
    progression_container.classList.add('progressions');
    this.container.appendChild(progression_container);
    let title = document.createElement('h1');
    ...
    Object.keys(this.progressions).forEach((prog_name) => {
      ...
      if(prog_name === this.current_progression) el.classList.add('progression-current');
      el.innerHTML = prog_name.replace('_', ' ');
      el.addEventListener('click', (e) => { this.updateProgression(e); });
      progression_container.appendChild(el);
    });
  };
  
  _loadChordStyleSelector() {
    let chord_style_container = document.createElement('section');
    chord_style_container.classList.add('chord-style');
    this.container.appendChild(chord_style_container);
    let title = document.createElement('h1');
    ...
    styles.forEach(style => {
      ...
      if(style.key === this.chord_style) el.classList.add('chord-style-current');
      el.innerHTML = style.label;
      el.addEventListener('click', (e) => { this.updateChordStyle(e); });
      chord_style_container.appendChild(el);
    });
  };
  
  _loadSongComposer() {
    let song_container = document.createElement('section');
    song_container.classList.add('song-composer');
    this.container.appendChild(song_container);
    let title = document.createElement('h1');
    title.innerHTML = 'Song Structure';
    song_container.appendChild(title);
    
    Object.keys(this.song_presets).forEach((preset_key) => {
      let preset = this.song_presets[preset_key];
      let el = document.createElement('div');
      el.setAttribute('data-value', preset_key);
      if(preset_key === this.current_song_preset) el.classList.add('song-preset-current');
      el.innerHTML = preset.name;
      el.addEventListener('click', (e) => { this.updateSongPreset(e); });
  ...
  randomizeAll() {
    ...
    setTimeout(() => {
      try {
        ...
        document.querySelectorAll('.keys div').forEach(el => el.classList.remove('key-current'));
        const keyElement = document.querySelector(`.keys div[data-value="${randomKey}"]`);
        ...
        document.querySelectorAll('.modes div').forEach(el => el.classList.remove('mode-current'));
        const modeElement = document.querySelector(`.modes div[data-value="${randomMode}"]`);
        ...
        this.chord_count = this.chords.length;
        document.querySelectorAll('.progressions div').forEach(el => el.classList.remove('progression-current'));
        const progressionElement = document.querySelector(`.progressions div[data-value="${randomProgression}"]`);
        if (progressionElement) progressionElement.classList.add('progression-current');
        
        // Randomize chord style
        const chordStyles = ['arpeggiated', 'block_chords'];
        const randomChordStyle = chordStyles[Math.floor(Math.random() * chordStyles.length)];
        this.chord_style = randomChordStyle;
        document.querySelectorAll('.chord-style div').forEach(el => el.classList.remove('chord-style-current'));
        const chordStyleElement = document.querySelector(`.chord-style div[data-value="${randomChordStyle}"]`);
        if (chordStyleElement) chordStyleElement.classList.add('chord-style-current');
        ...
        document.querySelectorAll('.song-composer div').forEach(el => el.classList.remove('song-preset-current'));
        const presetElement = document.querySelector(`.song-composer div[data-value="${randomPreset}"]`);
        ...
        document.querySelectorAll('.bpm div').forEach(el => el.classList.remove('bpm-current'));
        const bpmElement = document.querySelector(`.bpm div[data-value="${randomBPM}"]`);
        ...
        document.querySelectorAll('.steps div').forEach(el => el.classList.remove('step-current'));
        const stepsElement = document.querySelector(`.steps div[data-value="${randomSteps}"]`);
        ...
        document.querySelectorAll('.type div').forEach(el => el.classList.remove('type-current'));
        const typeElement = document.querySelector(`.type div[data-value="${randomType}"]`);
  ...
  exportAppState() {
    ...
    this.export_state_button.disabled = true;
    ...
    setTimeout(() => {
      ...
      this.progress_info.style.display = 'none';
      this.export_state_button.disabled = false;
    }, 100);
  };
  ...
  previewClaudeSong() {
    ...
    this.claude_preview_button.innerHTML = '🎵 Loading My Song...';
    this.claude_preview_button.disabled = true;
    ...
    setTimeout(async () => {
      ...
      this.claude_preview_button.innerHTML = '🎼 Now Playing: My Song';
      ...
      this.claude_preview_button.disabled = false;
  ...
  loadClaudeComposition(forceNew = false) {
    ...
    const now = new Date();
    const timeSeed = forceNew ? now.getTime() % 10000 : (this.claudeCompositionSeed || now.getTime() % 10000);
    ...
    const claudeKey = availableKeys[Math.floor((timeSeed * phi) % availableKeys.length)];
    
    // 2. Select mode based on mathematical aesthetics
    const claudeModes = ['dorian', 'lydian', 'mixolydian', 'aeolian']; // My preferred modes
    const claudeMode = claudeModes[Math.floor((timeSeed * phi * phi) % claudeModes.length)];
    
    // 3. Select progression - prefer complex mathematical patterns
    const claudeProgressions = ['glass_2', 'glass_3', 'minimalist']; // Most algorithmic
    const claudeProgression = claudeProgressions[Math.floor((timeSeed * phi * phi * phi) % claudeProgressions.length)];
    
    // 4. Select chord style based on section complexity
    const claudeChordStyle = Math.sin(timeSeed * 0.001) > 0 ? 'arpeggiated' : 'block_chords';
    
    // 5. Select song structure - prefer mathematical proportions  
    const claudeSongStructure = 'classic_glass'; // Most mathematical structure
    
    // 6. Select BPM - breathing, organic tempo
    const claudeBPM = Math.floor(120 + (Math.sin(timeSeed * 0.002) * 25)); // 95-145 range
    
    // 7. Select arpeggio steps - fibonacci-inspired
    const fibonacciSteps = [3, 5]; // Fibonacci numbers in our range
    const claudeSteps = fibonacciSteps[Math.floor((timeSeed * phi) % fibonacciSteps.length)];
    ...
    const claudePatternId = primes[Math.floor((timeSeed * phi) % primes.length)] % this.AP.patterns[claudePatternType].length;
    ...
    document.querySelectorAll('.keys div').forEach(el => el.classList.remove('key-current'));
    const keyElement = document.querySelector(`.keys div[data-value="${claudeKey}"]`);
    if (keyElement) keyElement.classList.add('key-current');
    
    // Update mode
    this.ms_mode = claudeMode;
    document.querySelectorAll('.modes div').forEach(el => el.classList.remove('mode-current'));
    const modeElement = document.querySelector(`.modes div[data-value="${claudeMode}"]`);
    if (modeElement) modeElement.classList.add('mode-current');
    ...
    this.chord_count = this.chords.length;
    document.querySelectorAll('.progressions div').forEach(el => el.classList.remove('progression-current'));
    const progressionElement = document.querySelector(`.progressions div[data-value="${claudeProgression}"]`);
    if (progressionElement) progressionElement.classList.add('progression-current');
    
    // Update chord style
    this.chord_style = claudeChordStyle;
    document.querySelectorAll('.chord-style div').forEach(el => el.classList.remove('chord-style-current'));
    const chordStyleElement = document.querySelector(`.chord-style div[data-value="${claudeChordStyle}"]`);
    if (chordStyleElement) chordStyleElement.classList.add('chord-style-current');
    
    // Update song structure
    this.current_song_preset = claudeSongStructure;
    this.composition_structure = this.song_presets[claudeSongStructure].structure;
    document.querySelectorAll('.song-composer div').forEach(el => el.classList.remove('song-preset-current'));
    const songElement = document.querySelector(`.song-composer div[data-value="${claudeSongStructure}"]`);
    ...
    const bpmElement = document.querySelector(`.bpm div[data-value="${closestBPM}"]`);
    if (bpmElement) bpmElement.classList.add('bpm-current');
    
    // Update arpeggio steps
    this.ap_steps = claudeSteps;
    document.querySelectorAll('.steps div').forEach(el => el.classList.remove('step-current'));
    const stepsElement = document.querySelector(`.steps div[data-value="${claudeSteps}"]`);
    if (stepsElement) stepsElement.classList.add('step-current');
    
    // Update pattern type
    this.ap_pattern_type = claudePatternType;
    document.querySelectorAll('.type div').forEach(el => el.classList.remove('type-current'));
    const typeElement = document.querySelector(`.type div[data-value="${claudePatternType}"]`);
    if (typeElement) typeElement.classList.add('type-current');
    ...
    setTimeout(() => {
      this.progress_info.innerHTML = '<h3>🎼 Claude\'s Composition Loaded</h3><p>Key: <strong>' + claudeKey + '</strong> | Mode: <strong>' + claudeMode + '</strong> | Style: <strong>' + claudeChordStyle + '</strong></p>';
      this.progress_info.style.display = 'block';
  ...
  generateMIDI() {
    ...
    setTimeout(() => {
      ...
      this.progress_info.style.display = 'none';
      this.midi_button.disabled = false;
    }, 100);
  };
  ...
  createClaudeSongMIDI() {
    ...
    const myKey = ['C', 'D', 'E', 'F#', 'G#', 'A#'][Math.floor((timeSeed * phi) % 6)]; // Algorithmic key selection
    ...
    while (currentTime < duration) {
      ...
      this.addClaudeVoices(tracks, sectionInfo, step, midiTime, sixteenthNote, myScale, fibonacci, primes, phi, timeSeed);
  ...
  createAppStateMIDI() {
    ...
    while (currentTime < this.midi_config.duration) {
      ...
      let section = this.composition_structure[current_section_idx];
      
      // Get current chord using the SAME logic as live transport
      let curr_chord = chord_step % this.chord_count;
      let chord = this.MS.notes[this.chords[curr_chord]]; // Use actual scale notes
      ...
      let pattern_variation = this.getPatternVariation(section, section_step);
      let note_index = (this.arpeggio[player_step % this.arpeggio.length] + pattern_variation) % notes.length;
      let note = notes[note_index];
      
      const midiTime = Math.floor(currentTime * ticksPerSecond);
      const velocity = Math.floor(this.getDynamicVelocity(section, section_step) * 127);
      
      // Bass logic - EXACT same as live transport
      let bass_o = chord.rel_octave + 2;
      let bass_note_name = chord.note + bass_o;
      
      if(!bass_on && this.shouldPlayBass(section)) {
        bass_on = true;
        let bass_velocity = section.layers > 1 ? Math.min(127, velocity + 20) : Math.min(127, velocity + 10);
        let bassMidi = this.noteToMidi[chord.note] + (bass_o) * 12;
        this.addMIDINote(tracks[1], bassMidi, midiTime, sixteenthNoteTicks * 8, bass_velocity);
      }
      ...
      if (this.chord_style === 'block_chords' && section.layers > 1) {
        ...
        let stepInMeasure = player_step % 16;
        if (stepInMeasure % 4 === 0) {
          chord.triad.notes.forEach((chordNote, i) => {
            let chordMidi = this.noteToMidi[chordNote.note] + (chordNote.rel_octave + this.player.octave_base) * 12;
            this.addMIDINote(tracks[0], chordMidi, midiTime + (i * 20), sixteenthNoteTicks * 2, Math.floor(velocity * 0.8));
            // Add harmony layer
            if (section.layers > 2) {
              this.addMIDINote(tracks[2], chordMidi, midiTime + (i * 30), sixteenthNoteTicks, Math.floor(velocity * 0.6));
            }
          });
        }
      } else {
        // Arpeggiated logic - same as live transport
        let note_ref_midi = this.noteToMidi[note.note] + (note.rel_octave + this.player.octave_base) * 12;
        this.addMIDINote(tracks[0], note_ref_midi, midiTime, sixteenthNoteTicks, velocity);
        
        // Add harmony layers for complexity
        if (section.layers > 2 && player_step % 2 === 0) {
          this.addMIDINote(tracks[2], note_ref_midi + 12, midiTime, sixteenthNoteTicks, Math.floor(velocity * 0.5));
        }
        if (section.layers > 3 && player_step % 3 === 0) {
          this.addMIDINote(tracks[3], note_ref_midi - 12, midiTime, sixteenthNoteTicks, Math.floor(velocity * 0.4));
        }
      }
      ...
      if(section_step >= section.duration * 4) { // 4 steps per beat
        current_section_idx = (current_section_idx + 1) % this.composition_structure.length;
        section_step = 0;
      }
  ...
  createMIDIComposition() {
    ...
    while (currentTime < this.midi_config.duration) {
      const currentSection = this.composition_structure[midiCurrentSection % this.composition_structure.length];
      
      if (currentSection && currentSection.layers > 0) {
        // RANDOM CHORD PROGRESSION - Mix up the order sometimes
        const baseChordChangeInterval = this.getChordChangeInterval(currentSection);
        const randomChordInterval = baseChordChangeInterval + (Math.floor(Math.random() * 3) - 1); // ±1 variation
        const chordIndex = Math.floor(step / Math.max(1, randomChordInterval)) % this.chords.length;
        
        // RANDOM CHORD SELECTION - Sometimes skip ahead or back
        let actualChordIndex = chordIndex;
        if (Math.random() < 0.15) { // 15% chance of surprise chord
          actualChordIndex = (chordIndex + (Math.random() < 0.5 ? 1 : -1) + this.chords.length) % this.chords.length;
        }
        ...
        const timingVariation = (Math.random() - 0.5) * (sixteenthNoteTicks * 0.1); // ±10% timing variation
        const humanizedTime = Math.max(0, midiTime + timingVariation);
        
        // RANDOM VELOCITY VARIATIONS - More dynamic expression
        const baseVelocity = Math.floor(this.getDynamicVelocity(currentSection, midiSectionStep) * 127);
        const velocityVariation = Math.floor((Math.random() - 0.5) * 30); // ±15 velocity variation
        const randomVelocity = Math.max(20, Math.min(127, baseVelocity + velocityVariation));
        
        this.addRandomNotesToMIDITracks(tracks, currentSection, chord, stepInMeasure, humanizedTime, sixteenthNoteTicks, step, randomVelocity, patternShuffle, chordInversions, rhythmVariations, textureChoices);
      ...
      if (Math.random() < 0.2) { // 20% chance of section variation
      ...
      midiSectionStep++;
      if(midiSectionStep >= sectionDuration * 4) { // 4 steps per beat
        midiCurrentSection = (midiCurrentSection + 1) % this.composition_structure.length;
        midiSectionStep = 0;
      }
  ...
  generateClaudeScale(timeSeed) {
    ...
    const selectedScale = baseScales[Math.floor((timeSeed * 1.618) % baseScales.length)];
  ...
  generateClaudeStructure(phi, fibonacci, totalDuration) {
    ...
    const sections = [
      ...
      },
      {
        ...
        complexity: 0.9,
        description: 'Full complexity emerges from simple mathematical relationships'
      },
      {
      ...
      },
      {
        ...
        complexity: 0.3,
        description: 'Recognition of the infinite patterns within finite expression'
      }
    ];
    
    // Calculate actual durations based on proportions
    const totalProportion = sections.reduce((sum, section) => sum + section.proportion, 0);
    let currentTime = 0;
  ...
  addClaudeVoices(tracks, section, step, midiTime, duration, scale, fibonacci, primes, phi, timeSeed) {
    ...
    if (section.complexity > 0.3) {
      ...
      if (step % primes[primeIndex] === 0) {
        ...
        this.addMIDINote(tracks[1], harmonicNote, midiTime, duration * 4, Math.floor(velocity * 0.6));
      }
    }
    ...
    if (section.complexity > 0.4) {
      ...
      if (step % 8 === 0) { // Every 8th step
        ...
        this.addMIDINote(tracks[2], note + (octave * 12), midiTime, duration * 2, Math.floor(velocity * 0.7));
      }
      ...
      if (step % 4 === 0 && Math.sin(step * 0.1) > 0.5) {
        ...
        this.addMIDINote(tracks[2], note + (octave * 12), midiTime, duration, Math.floor(velocity * 0.4));
      }
    }
    ...
    if (section.complexity > 0.6) {
      ...
      const chaos = Math.sin(step * 0.1) * Math.cos(step * 0.07) * Math.sin(step * 0.13);
      if (Math.abs(chaos) > 0.8) { // Emergent moments
        const emergentNote = scale[Math.floor(Math.abs(chaos * 100) % scale.length)];
        const octave = octaveBase + Math.floor(Math.abs(chaos * 3));
        this.addMIDINote(tracks[3], emergentNote + (octave * 12), midiTime, duration * 3, Math.floor(velocity * 0.5));
      }
    }
    ...
    if (section.complexity > 0.5) {
      ...
      if (step % 16 === 0 && Math.abs(timeFunction) > 1) {
        const reflectedNote = scale[(step % scale.length + Math.floor(Math.abs(timeFunction * 10))) % scale.length];
        const octave = octaveBase + Math.floor((timeSeed * step * 0.001) % 3);
        this.addMIDINote(tracks[4], reflectedNote + (octave * 12), midiTime, duration * 6, Math.floor(velocity * 0.3));
      }
    }
    ...
    if (step % Math.floor(fibonacci[7]) === 0 && section.complexity > 0.7) {
      ...
      scale.forEach((note, index) => {
        if (index < 3) { // Triad
          ...
          this.addMIDINote(tracks[1], harmonyNote, midiTime + (index * 100), duration * 8, Math.floor(velocity * 0.8));
        }
      });
    }
  };
  ...
  getCurrentMIDISectionAt(time) {
    ...
    let totalTime = 0;
    for (let section of this.composition_structure) {
  ...
  getMIDIScale() {
    ...
    const scaleIntervals = this.MS._scale.steps.map(step => (keyIndex + step) % 12);
  ...
  addRandomNotesToMIDITracks(tracks, section, chord, stepInMeasure, midiTime, duration, globalStep, velocity, patternShuffle, chordInversions, rhythmVariations, textureChoices) {
    ...
    if (globalStep % this.getRandomBassInterval(currentTexture.bassPattern, section) === 0) {
      const bassInversion = currentInversion.bass;
      const bassNote = chord.triad.notes[bassInversion];
      let bassMidi = this.noteToMidi[bassNote.note] + (bassNote.rel_octave + 2 + currentInversion.octaveShift) * 12;
      ...
      this.addMIDINote(tracks[2], bassMidi, midiTime, bassDuration, Math.min(127, velocity + 15));
    }
    
    // TRACK 0: LEAD - Random arpeggio patterns
    if (Math.random() < currentRhythm.density) { // Random note density
      ...
      if (noteIndex < chord.triad.notes.length) {
        ...
        let leadMidi = this.noteToMidi[selectedNote.note] + (selectedNote.rel_octave + this.player.octave_base + currentInversion.octaveShift) * 12;
        ...
        if (Math.random() < currentRhythm.doubleChance) {
          ...
          this.addMIDINote(tracks[0], leadMidi, midiTime + duration, duration, Math.floor(velocity * 0.8));
        } else if (Math.random() > currentRhythm.skipChance) { // Random note skipping
          const noteDuration = duration * (Math.random() * 1.5 + 0.5); // Vary note length
          this.addMIDINote(tracks[0], leadMidi, midiTime, noteDuration, velocity);
        }
      }
    }
    ...
    if (section.layers >= 2) {
      if (currentTexture.harmonyStyle === 'block') {
        ...
        if (stepInMeasure % 3 === 0 && Math.random() < 0.6) {
          const randomChordNote = chord.triad.notes[Math.floor(Math.random() * chord.triad.notes.length)];
          let harmonyMidi = this.noteToMidi[randomChordNote.note] + (randomChordNote.rel_octave + this.player.octave_base) * 12;
          this.addMIDINote(tracks[1], harmonyMidi, midiTime, eighthNote, Math.floor(velocity * 0.6));
        }
      }
      
      // Random octave doubling
      if (currentTexture.octaveDoubling && stepInMeasure % 8 === 0) {
        const rootMidi = this.noteToMidi[chord.note] + (chord.rel_octave + this.player.octave_base + 1) * 12;
        this.addMIDINote(tracks[1], rootMidi, midiTime, quarterNote * 2, Math.floor(velocity * 0.5));
      }
    }
    ...
    if (section.layers >= 3 && currentTexture.textureLayer) {
      if (currentTexture.counterMelody && stepInMeasure % 5 === 0) {
        ...
        const randomScaleNote = scaleNotes[Math.floor(Math.random() * scaleNotes.length)];
        let textureMidi = this.noteToMidi[randomScaleNote.note] + (randomScaleNote.rel_octave + this.player.octave_base + 1) * 12;
        ...
        this.addMIDINote(tracks[3], textureMidi, midiTime, eighthNote, Math.floor(velocity * 0.4));
      }
      
      // Random atmospheric holds
      if (Math.random() < 0.1 && stepInMeasure === 0) { // 10% chance per measure
        const holdNote = chord.triad.notes[Math.floor(Math.random() * chord.triad.notes.length)];
        let holdMidi = this.noteToMidi[holdNote.note] + (holdNote.rel_octave + this.player.octave_base + 2) * 12;
        this.addMIDINote(tracks[3], holdMidi, midiTime, halfNote * 2, Math.floor(velocity * 0.3));
      }
    }
    ...
    if (globalStep % currentRhythm.accentPattern === 0) {
      ...
      let accentMidi = this.noteToMidi[accentNote.note] + (accentNote.rel_octave + this.player.octave_base) * 12;
      this.addMIDINote(tracks[0], accentMidi, midiTime, duration * 1.5, Math.min(127, velocity + 20));
    }
  };

  addNotesToMIDITracks(tracks, section, chord, rootNote, stepInMeasure, midiTime, duration, globalStep) {
    const quarterNote = duration * 4;
    const eighthNote = duration * 2;
    const halfNote = duration * 8;
    
    // Get velocity from the same function used in real-time
    const velocity = Math.floor(this.getDynamicVelocity(section, globalStep) * 127);
    ...
    const arpeggio_index = (globalStep + pattern_variation) % this.arpeggio.length;
    ...
    if (this.shouldPlayBass(section) && globalStep % this.getChordChangeInterval(section) === 0) {
      const bassNote = this.noteToMidi[chord.note] + (chord.rel_octave + 2) * 12 + 24; // Bass octave
      this.addMIDINote(tracks[2], bassNote, midiTime, halfNote, Math.min(127, velocity + 20));
    }
    ...
    if (this.chord_style === 'block_chords' && section.layers > 1) {
      ...
      if (arpeggio_note_index < notes.length) {
        ...
        const leadNote = this.noteToMidi[selectedNote.note] + (selectedNote.rel_octave + this.player.octave_base) * 12;
      ...
      if (section.layers >= 2 && stepInMeasure % 4 === 0) {
        chord.triad.notes.forEach((chordNote, i) => {
          const harmonyNote = this.noteToMidi[chordNote.note] + (chordNote.rel_octave + this.player.octave_base) * 12;
          this.addMIDINote(tracks[1], harmonyNote, midiTime + (i * 40), quarterNote, Math.floor(velocity * 0.75));
        });
      }
      ...
      if (section.layers >= 3 && stepInMeasure % 6 === 0) {
        ...
        if (textureIndex < notes.length) {
          ...
          const textureMidi = this.noteToMidi[textureNote.note] + (textureNote.rel_octave + this.player.octave_base + 1) * 12;
          this.addMIDINote(tracks[3], textureMidi, midiTime, eighthNote, Math.floor(velocity * 0.65));
        }
      }
      ...
      if (section.name === 'complexity' && stepInMeasure % 3 === 0) {
        ...
        if (complexityIndex < notes.length) {
          ...
          const complexityMidi = this.noteToMidi[complexityNote.note] + (complexityNote.rel_octave + this.player.octave_base + 1) * 12;
          this.addMIDINote(tracks[3], complexityMidi, midiTime + (duration * 0.5), duration, Math.floor(velocity * 0.55));
        }
      }
    }
    ...
    if (section.name === 'complexity' && stepInMeasure % 12 === 0) {
      const rootMidi = this.noteToMidi[chord.note] + (chord.rel_octave + this.player.octave_base - 1) * 12;
      this.addMIDINote(tracks[1], rootMidi, midiTime, quarterNote * 3, Math.floor(velocity * 0.7));
    }
  };
  ...
  downloadMIDI(midiData, exportType = 'app_state') {
    ...
    const chordStyleShort = this.chord_style === 'block_chords' ? 'block' : 'arp';
    const progressionShort = this.current_progression.replace('glass_', 'g').replace('_', '');
    const songStructureShort = this.current_song_preset.replace('_', '').substring(0, 6);
    ...
    if (exportType === 'app_state') {
      ...
      // Original filename for preset export
      filename = `glass_${this.ms_key}_${this.ms_mode}_${this.player.bpm}bpm_${chordStyleShort}_${progressionShort}_${songStructureShort}_${this.ap_steps}steps.mid`;
    }
  ...
  _updateChordSelector() {
    ...
    let old_selector = document.querySelector('.chord');
  ...
  getDynamicVelocity(section, step) {
    ...
    let base_velocity = 0.7;
    let section_modifier = {
      'intro': 0.5,
      'building': 0.7,
      'complexity': 0.9,
      'reduction': 0.6,
    ...
    let phrase_position = (step % 32) / 32; // 8-measure phrases
    let phrase_curve = Math.sin(phrase_position * Math.PI) * 0.2;
